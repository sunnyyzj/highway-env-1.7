...
class HighwayEnvBS(HighwayEnvFast):
    ...
    def default_config(cls) -> dict:
        ...
        conf.update({
            ...
            'rf_bs_count': 5,  #20 5 10 
            'thz_bs_count': 0,  #100 20
            'rf_bs_max_connections': 5,  # 最大连接数量 5
    ...
    def _agent_rewards(self, action: int, vehicle: Vehicle) -> Dict[Text, float]:
        ...
        if vehicle.target_current_bs is not None:
            ...
            result_rf = utils.lmap(result_rf,[0, self.config["tele_reward_threshold"]],[0, 200])#1e8
            # result_rf = utils.lmap(result_rf,[-300, -200],[0, 2])#1e8
            # result_rf = utils.lmap(result_rf,[0, 4e5],[0, 2])#1e8
...
# class Highway3DEnvBS(HighwayEnvFast):
...
#             # obstacle_lane = 0
#             obstacle_dist = np.random.randint(300, 10000)
#             self.road.objects.append(Obstacle(self.road, [obstacle_dist, obstacle_lane, 0]))
...
#                 vehicle_lane = np.random.choice(lanes)
#                 vehicle_dist += 25
#                 vehicle.position = np.array([vehicle_dist, vehicle_lane, 100])
#                 self.controlled_vehicles.append(vehicle)
#                 self.road.vehicles.append(vehicle)
#                 # self.shared_state.vehicles.append(vehicle)  # shared state append
#             else:
...
#             result_rf = utils.lmap(result_rf,[-100, -80],[0, 2])#1e8
#             # result_rf = "{:.2f}".format(result_rf)
#             # print('final result_rf',result_rf)
